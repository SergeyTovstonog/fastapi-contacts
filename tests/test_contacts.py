...
@pytest.mark.asyncio
async def test_get_contact(override_get_db, test_user_contact: Contact, auth_headers, monkeypatch):
    mock_cache = AsyncMock()
    monkeypatch.setattr("src.contacts.routers.invalidate_contacts_cache", mock_cache)
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:

        # Then, retrieve the contact by ID
        response = await ac.get(f"/contacts/{test_user_contact.id}", headers=auth_headers)
